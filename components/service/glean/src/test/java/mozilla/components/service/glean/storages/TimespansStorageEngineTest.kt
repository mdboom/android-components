...
class TimespansStorageEngineTest {
    ...
    fun `timespan deserializer should correctly parse JSONArray(s)`() {
        ...
        val snapshot = storageEngine.getSnapshotWithTimeUnit(storeName = "store1", clearStore = true).metrics!!
    ...
    fun `a single elapsed time must be correctly recorded`() {
        ...
        val snapshot = spiedEngine.getSnapshotWithTimeUnit(storeName = "store1", clearStore = false).metrics!!
        assertEquals(1, snapshot.size)
    ...
    fun `the recorded time must conform to the chosen resolution`() {
        ...
        expectedResults.forEach { (res, expectedTimespan) ->
            ...
            val snapshot = spiedEngine.getSnapshotWithTimeUnit(storeName = "store1", clearStore = true).metrics!!
            assertEquals(1, snapshot.size)
