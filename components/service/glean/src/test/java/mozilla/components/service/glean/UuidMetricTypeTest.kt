...
class UuidMetricTypeTest {
    ...
    fun `The API saves to its storage engine`() {
        ...
        val snapshot = UuidsStorageEngine.getSnapshot(storeName = "store1", clearStore = false).metrics!!
        ...
        // Check that data was properly recorded.
        val snapshot2 = UuidsStorageEngine.getSnapshot(storeName = "store1", clearStore = false).metrics!!
    ...
    fun `uuids with no lifetime must not record data`() {
        ...
        assertNull("Uuids must not be recorded if they are disabled", snapshot.metrics)
    }
